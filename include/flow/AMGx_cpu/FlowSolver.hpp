        ...
        public:
            ...
            FlowSolver(MPI_Config *mpi_config, Mesh<T> *mesh, double delta) : mesh(mesh), delta(delta), mpi_config(mpi_config)
            {
				...
				AMGX_resources_create(&main_rsrc, cfg, 
										&(mpi_config->particle_flow_world), 1, {0});
				...
				values = (T *) calloc(mesh->local_mesh_size*7, sizeof(T));
				col_indices = (int64_t *) calloc(mesh->local_mesh_size*7, sizeof(int64_t));
				rows_ptr = (int *) calloc(mesh->local_mesh_size+1, sizeof(int));
