...
namespace minicombust::particles 
{
    ...
    void ParticleSolver<T>::output_data(uint64_t timestep)
    {
        ...
        VisitWriter<double> *vtk_writer = new VisitWriter<double>(mesh, current_particle0, particles);
    ...
    void ParticleSolver<T>::timestep()
    {
        ...
        static int  count = 0;
        if ((count++ % 20) == 0)  cout << "\tTimestep " << count-1 << ". Particles in simulation: " << logger.num_particles - logger.decayed_particles << " max " << (mesh->max_cell_particles * mesh->mesh_size) << endl;
        // update_flow_field();
