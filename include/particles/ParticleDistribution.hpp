        ...
        public:
            ...
            {   
                ...
                if (dist == UNIFORM)
                {
                    ...
                    cyclindrical_velocity_lower.y = (mpi_config->particle_flow_rank + 0) * (2 * M_PI / mpi_config->particle_flow_world_size);
                    cyclindrical_velocity_upper.y = (mpi_config->particle_flow_rank + 1) * (2 * M_PI / mpi_config->particle_flow_world_size);
            ...
            inline void emit_particles_evenly(vector<Particle<T>>& particles, vector<unordered_map<uint64_t, uint64_t>>& cell_particle_field_map, unordered_map<uint64_t, flow_aos<T> *>& node_to_field_address_map,  uint64_t **indexes, particle_aos<T> **indexed_fields, function<void(uint64_t*, uint64_t, uint64_t ***, particle_aos<T> ***)> resize_fn, Particle_Logger *logger)
            {
                ...
                for (uint64_t p = 0; p < even_particles_per_timestep + remainder; p++)
                {
                    ...
                    if ( !cell_particle_field_map[block_id].count(particle.cell) )
                    {
                        ...
                        resize_fn(elements, block_id, &indexes, &indexed_fields);
                        ...
                        for (uint64_t n = 0; n < mesh->cell_size; n++)
                        {
                            ...
                            {
                                node_to_field_address_map[node_id] = (flow_aos<T> *)1;
                            }
                        }
                    }
                }
